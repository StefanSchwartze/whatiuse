  ...
  async render(Handler: object, flux: object, force: ?boolean = false) {
    if (process.env.BROWSER && !force) {
      ...
      try {
        ...
        // ReactDOM.renderToString(React.createElement(Handler, {flux}));
        ...
        const app: string = ReactDOM.renderToString(React.createElement(Handler, {flux}));
        ...
        debug('koa')(error);
        const app: string = ReactDOM.renderToString(React.createElement(ErrorPage));
        content = Iso.render(app, flux.flush());
      }
